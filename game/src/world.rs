...
pub struct World {
    ...
    legs_computed: u64,
    /// Скільки ланок позаду курсора лишати сирими (`set_retirement`).
...
impl World {
    ...
    /// Скільки ланок позаду курсора лишати сирими, або `None` — не
    /// відправляти на пенсію нікого (N3a).
    ///
    /// Політика, а не налаштування, і в неї два законні значення в самому
    /// проєкті. Гра відправляє: інакше пам'ять росте так, як каже D7.
    /// Не відправляють двоє — спекулятивний світ планувальника, який живе
    /// кілька ланок і встиг би хіба заплатити за прохід, і будь-яка перевірка,
    /// що звіряє **потік семплів** із незалежним прогоном: пенсія викидає
    /// семпли, тобто змінює те, що така перевірка порівнює, не змінюючи
    /// жодного біта того, що порахували.
    pub fn set_retirement(&mut self, behind_legs: Option<usize>) {
