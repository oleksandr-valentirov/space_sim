...
//!
//! ## Чого тут немає: зовнішньої задачі
//!
//! Обидві сторони цієї звірки роблять `v += Δv` за однією формулою, тож про
//! **фізичний сенс** імпульсу вона не каже нічого — і не має. Це предмет
//! `game/tests/impulse.rs` (ROADMAP L4, борг D1): там Ламберт із межі дає
//! наближення, `prop_run_stm` його виправляє, апарат прилітає в позицію
//! Місяця з ассета, а базис VNB перевіряється підручниковою двотільною
//! механікою. Борг, який тут раніше був записаний як відкритий, закритий
//! саме там.
