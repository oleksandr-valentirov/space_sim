...
// Одна з двох функцій межі поза зоною детермінізму (друга —
// porkchop_compute_eph), і перша, що бере структуру ЗА ЗНАЧЕННЯМ (Vec3d —
// 24 байти, тобто через пам'ять на всіх наших ABI). Живе в окремій
// libcore_planning.a і має ВЛАСНИЙ оракул, що лінкується з -lm.
...
- Функція межі втратила останнього викликача (як `porkchop_compute_eph`,
  борг D10) — це привід не лишати її мовчки, а назвати в ROADMAP умову
  видалення.
- З'явився новий шматок C API з PROJECT.md §5 — додай виклик у той оракул,
  до якого функція належить за libm.
