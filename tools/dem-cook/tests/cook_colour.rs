...
/// Спільне ребро двох патчів несе той самий байт.
///
/// Це і є «між тайлами немає шва»: останній вузол лівого — це нульовий вузол
/// правого, і кукер бере їх у бітово тому самому напрямку. Падає воно, якщо
/// координати всередині патча зсунулись.
///
/// ⚠ **Половину цього тесту прибрав етап W** (крок W4): була ще звірка ореолу
/// зі сіткою сусіда, і вона перевіряла пошук сусіда за ребром куба. Ореолу в
/// колірному тайлсеті більше немає — його там ніколи не читали, — тож
/// перевіряти нема чого. Той самий пошук лишається перевіреним у рельєфу
/// (`cook.rs::the_halo_holds_the_neighbours_own_node`), де ореол справді
/// вживається.
#[test]
