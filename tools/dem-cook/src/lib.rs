...
/// У скільки разів грубішає сітка на наступному кроці ланцюга, або `None`,
/// коли ділити далі нема чого.
///
/// ## Чому не завжди вдвічі
///
/// Ланцюг (T3c) існує заради грубих рівнів піраміди: вузол рівня 0 накриває
/// 312 км Землі, і точкова вибірка з сітки в 1.85 км обирає з тридцяти тисяч
/// пікселів один навмання. Тому ділити треба доти, доки піксель не дійде до
/// вузла найгрубішого рівня.
///
/// Ділення надвоє цього не забезпечує: **10800 = 2⁴ · 675**, тобто ланцюг
/// Землі зупинявся б на 1350×675 — 29.7 км на піксель проти 312 км вузла,
/// удесятеро дрібніше. Місячна сітка (5760×2880 = 2⁶ · 45) ділиться довше й
/// сховала б це від нас: там ланцюг доходить куди треба сам.
///
/// Тому крок — **найменший дільник обох сторін** з трьох простих. Рівномірності
/// ланцюга це не потребує: рівень піраміди питає в нього сітку **за кутом**
/// (`cook::source_for`), а не за номером, тож нерівний крок нічого не ламає.
///
/// Три, а не «будь-який дільник»: більший крок міг би перескочити потрібну
/// грубість, а обмежувати вибір знизу нема чим — 675 = 3³ · 5², і без п'ятірки
/// ланцюг Землі став би на 450×225.
pub fn reduce_step(samples: usize, lines: usize) -> Option<usize> {
    // Нижче чотирьох рядків сітка вже не карта, а чотири числа.
    if lines < 8 {
        return None;
    }
    [2, 3, 5]
