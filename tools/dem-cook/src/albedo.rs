...
impl Albedo {
    ...
    /// Ланцюг сіток, кожна вдвічі грубіша за попередню; нульова — ця сама.
    ///
    /// ## Навіщо він узагалі, і чому це не косметика
    ///
    /// Кукер бере в кожному вузлі **точку**, і на дрібних рівнях це чесно:
    /// вузол там дрібніший за піксель джерела. На грубих — ні. Вузол рівня 0
    /// накриває 85 км Місяця, тобто дві тисячі пікселів мозаїки, і точкова
    /// вибірка обирає з них один навмання. Побачити це можна в демо:
    /// далекий Місяць виходив плямистим шумом замість карти, на якій видно
    /// моря.
    ///
    /// Ланцюг це знімає: рівень бере ту сітку, чий піксель уже не дрібніший за
    /// його вузол. Інваріант «вузол на спільному ребрі бітово один» при цьому
    /// цілий — вибір сітки залежить **лише від рівня**, тож обидва сусіди
