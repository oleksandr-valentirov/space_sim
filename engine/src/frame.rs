...
/// Колір очищення, **у лінійному світлі**. Не чорний навмисно: чорний кадр і
/// кадр, якого не було, виглядають однаково, і перевірка «щось намалювалось»
/// на чорному нічого не варта.
///
/// ⚠ Числа виглядають дивно й такі саме тому, що вони не підібрані, а
/// **виведені**: ціль кодує гамму (T5a), тож джерелом істини лишається
/// [`CLEAR_BYTES`] — той вигляд неба, який обрано оком, — а тут стоїть
/// `srgb::to_linear` від нього. Звіряє пару тест
/// `the_clear_colour_still_writes_the_bytes_it_promises`; розійтися їм не
/// можна, бо на `CLEAR_BYTES` стоїть половина перевірок рушія й гри
/// («це небо чи щось намальоване»).
...
/// Той самий колір у байтах знімка — і саме він тут первинний.
...
struct Uniforms {
    ...
    /// висоти, тобто `scale_m / radius_m`; `y` — масштаб кольору, `z` — його
