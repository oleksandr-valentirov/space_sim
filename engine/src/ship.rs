...
///
/// Не виміряна величина, а вибір вигляду: 0.40 — це емаль, у якої відблиск
/// уже не дзеркало, але ще й не матова пляма. Ця стала існує рівно для того,
/// щоб зонди й тести не розводили корпус на десять різних матеріалів.
pub const HULL_ROUGHNESS: f32 = 0.40;

/// Корпус **пофарбований**, тобто діелектрик: нуль, а не одиниця (T9b).
///
/// До T9 корпус був металом, і це було чесно, поки він був голим корпусом
/// одного кольору. Модель з референсу пофарбована емаллю, а фарба — це
/// діелектрик поверх металу: дифузне відбиття в неї є, і саме воно несе
/// колір. З `metallic = 1` базовий колір ставав би `F0`, тобто відтінком
/// самого відблиску, і біла емаль виглядала б темним хромом — рівно так вона
/// й виглядала на першому кадрі T9a.
///
/// Проміжних значень не буває фізично, тож нуль, а не 0.1 «щоб було
/// цікавіше»: матеріал або має вільні електрони, або ні.
