...
//! жодного з них не входить**. Тому геометрія тут породжується кодом, а не
//! читається з файлу: пайплайн мешів приходить із PBR, тобто з першим кроком,
//! якому потрібні матеріали, і заводити його наперед означало б завести
//! структуру без викликача (CLAUDE.md).
//!
