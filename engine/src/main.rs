...
fn run_perf_probe() -> Result<(), String> {
    ...
    // Окремо — ціна повітря (S5). Три висоти навколо умови пропуску об'єму
    // аеральної перспективи: 6.24·10⁷ м — та відстань, на якій сто кілометрів
    // шару займають рівно піксель кадру.
    println!(
        "\nПовітря (етап S). Умова S5: об'єм аеральної перспективи рахується, \n\
         лише коли товщина шару в кадрі не менша за піксель.\n"
    );
    println!(
        "{:>10} {:>12} {:>8} {:>8} {:>8}",
        "висота", "об'єм", "min мс", "mean мс", "p95 мс"
    );
    for (altitude, label) in [
        ...
        let stats = engine::perf_probe::aerial_cost(&gpu, 1280, 720, FRAMES, altitude)?;
        println!(
            "{:>10.1e} {:>12} {:>8.3} {:>8.3} {:>8.3}",
            altitude, label, stats.min_ms, stats.mean_ms, stats.p95_ms
        );
    }
