...
//!
//! Лишається одна домішка, і вона рахується: рампа нахиляє поверхню на
//! `atan(slope)`, тобто змінює дифузний член. При нахилі 0.05 це 2.9° і
//! 0.12% яскравості проти 5% від самого правила — тобто домішка в сорок разів
//! менша за сигнал, а не «нею знехтувано».
...
/// Нахил рампи. Дрібний навмисно — див. про домішку в шапці модуля.
const SLOPE: f64 = 0.05;

/// Метрів в одиниці зберігання.
///
/// Не одиниця: сталий нахил по всьому тілу неминуче накопичує рельєф
/// (0.05 на чверть великого кола — це 136 км), і в `i16` він влазить лише з
...
fn the_frame_shows_the_multiplier_the_rule_predicts() {
    ...
    assert!(
        ...
        "кадр дав {measured:.4} проти передбачених {predicted:.4} — правило в \
         шейдері розійшлося з `engine::material`"
    );
}
...
fn the_pyramid_depth_does_not_repaint_the_slope() {
    ...
    // Одиниця — це квант восьмибітної шкали; правило, що читало б крок
    // піраміди, дало б тут десятки.
    assert!(
        worst <= 1.0,
        "дві глибини піраміди дали різні кольори: до {worst:.1} одиниці"
    );
}
