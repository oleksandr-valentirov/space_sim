...
/// Зазор навмисно 100 м, а не 1 м, як у сусіднього тесту.
///
/// На 10⁷ м із зазором 1 м обидві поверхні лягають рівно на межу
/// float32-заокруглення: справжня (double) різниця їхніх `z_ndc` — частка
/// однієї ULP. На CPU-растеризаторі (`lvp`, Mesa) це дає чистий програш
/// звичайної проєкції, як і задокументовано в ROADMAP F3. На апаратному
/// Vulkan-драйвері (виміряно на `AMD RADV VANGOGH`, Steam Deck) той самий
/// випадок раптом **вигравала** звичайна проєкція — не тому, що вона стала
/// точнішою, а тому, що округлення на межі ULP — це вже деталь конкретного
/// растеризатора (fused-multiply-add у GPU-компіляторі, апаратний rcp замість
/// честивого ділення), а не гарантія IEEE-754. `-ffp-contract=off` з
/// CLAUDE.md рятує від цього класу проблем у C, але на WGSL-шейдери це
/// правило не поширюється — межа детермінізму (PROJECT.md §4) проходить по
/// пропагації, не по рендеру.
///
/// Зазор 100 м лишається в тому самому кадрі (10⁷ м), тож твердження
/// «звичайна проєкція тут категорично не працює» нікуди не зникло — просто
/// перевіряється не в точці, де самé питання «хто виграв» ще залежить від
/// того, яка залізка рахує.
#[test]
fn a_conventional_projection_loses_the_same_case_entirely() {
    let Some(share) = near_wins(false, 1e7, 100.0) else {
        return;
    };
    assert!(
        share < 0.01,
