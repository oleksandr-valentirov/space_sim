...
//!    
//! 2. **видимий патч не намальовано взагалі.** Крізь дірку видно не небо, а
//!    **той бік тієї самої планети**: відсікання задніх граней вимкнене
//!    свідомо (`cull_mode: None`), а дальня півсфера ще малюється. Пікселі
//!    міняють відтінок, але кольору неба не набувають.
//!
//! Отже зуби цьому детекторові дає **R3**: коли horizon culling перестане
//! малювати те, що за лімбом, дірка почне показувати саме небо, і мутацію 2
//! треба прогнати ще раз — уже там. Записано тут, щоб не забулось, і саме
//! в такому вигляді, як воно є: сьогодні це сторож проти грубого промаху
//! (не та грань, не той діапазон індексів), а не проти тріщини.
//!
